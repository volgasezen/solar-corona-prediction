...
def train(model, current_beta, current_lambda):
    ...
    for x in train_loader:
        ...
        loss_temp = spatio_temporal_loss(mu, T)
        total_loss = loss_recon + (current_beta * loss_kl) + (current_lambda * loss_temp)
        # Backward
        optimizer.zero_grad()
        total_loss.backward()
        optimizer.step()
        l.append([loss_kl.item(),loss_recon.item(),loss_temp.item()])
    return l
...
torch.save(model.state_dict(), '/home/oban/Desktop/Volga/MMI711/project/3-lstm2/model_conv_vae.pt')
