...
state_dict2 = torch.load('/home/oban/Desktop/Volga/MMI711/project/model_latent_convlstm_128.pt', weights_only=False)
...
class CNN_LSTM_CNN(nn.Module):
    ...
    def forward(self, x, y):
        ...
        x_feat = x_feat.view(batch_size, seq_len, 128, 8, 8)
        y_feat = y_feat.view(batch_size, seq_len//2, 128, 8, 8)

        preds = self.lstm(x_feat, z_target=y_feat)

        preds = preds.view(batch_size*seq_len//2, 128, 8, 8)
...
for i in range(100):
    ...
    if val_l * 1.01 < best_loss:
        torch.save(final_model.state_dict(), f'/home/oban/Desktop/Volga/MMI711/project/model_convlstm_finetuned_128_{i}.pt')
        print(f'{(best_loss-val_l)/best_loss:.2%} improvement in loss! Model checkpoint saved at Epoch {i}')
        best_loss = val_l
    if i%5 == 0:
...
torch.save(final_model.state_dict(), '/home/oban/Desktop/Volga/MMI711/project/model_convlstm_finetuned_128.pt')
