...
# %%
state_dict = torch.load('project/3-lstm2/model_conv_ae2.pt', weights_only=False)
...
latent1 = encode_frames(train_x, model)
# %%
n = len(latent1)
latent1 = latent1.reshape(n, -1)

import umap
reducer = umap.UMAP(random_state = 1336)
reducer.fit(latent1)
# %%
umap1 = reducer.transform(latent1)
angles = np.repeat(np.arange(8),6010)

plt.figure(figsize=(10,10), dpi=300)
plt.scatter(umap1[:, 0], umap1[:, 1], c=angles, s=2)
plt.gca().set_aspect('equal', 'datalim')
plt.colorbar(boundaries=np.arange(9)-0.5).set_ticks(np.arange(8))
plt.title('UMAP projection of patches embedded with Spatio-Temporal loss');
# %%
model2 = CNN_VAE()
state_dict = torch.load('project/3-lstm2/model_conv_ae3.pt', weights_only=False)
model2.load_state_dict(state_dict)
model2.cuda().eval()

latent2 = encode_frames(train_x, model2)
n = len(latent2)
latent2 = latent2.reshape(n, -1)

reducer = umap.UMAP(random_state = 1336)
reducer.fit(latent2)
# %%
umap2 = reducer.transform(latent2)

plt.figure(figsize=(10,10), dpi=300)
plt.scatter(umap2[:, 0], umap2[:, 1], c=angles, s=2)
plt.gca().set_aspect('equal', 'datalim')
plt.colorbar(boundaries=np.arange(9)-0.5).set_ticks(np.arange(8))
plt.title('UMAP projection of patches embedded with regular model');
