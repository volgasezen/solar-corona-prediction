...
import time
import sys
sys.path.append('/home/oban/Desktop/Volga/MMI711/PhyDNet')
...
constraints
# %%
root = '/home/oban/Desktop/Volga/MMI711/PhyDNet/data'
mm = MovingMNIST(root=root, is_train=True, n_frames_input=10, n_frames_output=10, num_objects=[2])
train_loader = torch.utils.data.DataLoader(dataset=mm, batch_size=128, shuffle=True, num_workers=0)

mm = MovingMNIST(root=root, is_train=False, n_frames_input=10, n_frames_output=10, num_objects=[2])
test_loader = torch.utils.data.DataLoader(dataset=mm, batch_size=128, shuffle=False, num_workers=0)
...
def evaluate(encoder,loader):
    ...
    with torch.no_grad():
        for i, out in enumerate(loader, 0):
            ...
            cross_entropy = cross_entropy / (128*target_length)
...
trainIters(encoder,2,print_every=1,eval_every=2,name='anan')
